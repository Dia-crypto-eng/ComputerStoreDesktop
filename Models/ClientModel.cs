...
namespace ComputerStore.Models
{
    internal class ClientModel :BaseModel
    {
        ...
        public string Email { get { return email; }  set { // تحقق من صحة البريد الإلكتروني باستخدام MailAddress
                try
                {
                    var mailAddress = new MailAddress(value); email = value; OnPropertyChanged(nameof(Email));
                }
                catch (FormatException ex)
                {
                    // التعامل مع الصيغة غير الصالحة
